<template>
    <el-dialog :visible.sync="visible" :title="data.title" :disabled="true" :show-close="false"
        ...
        <el-input placeholder="输入部门名称搜索" v-model="deptname" size="small"></el-input>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      submit() {
        let data = {roleId:this.data.roleId,targetType:1,targetIds:this.$refs.tree.getCheckedKeys()}
        save(data).then(()=>{
          this.$message({
            message: '操作成功',
            type: 'success'
